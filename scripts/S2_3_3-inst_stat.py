...
params = x3cflux.get_parameters(
    simulator_inst.parameter_space, config.parameter_entries
)
...
params_stat = x3cflux.get_parameters(
    simulator_stat.parameter_space, config_stat.parameter_entries
)
...
inst_names = {
    x.name: f"{x.metabolite_name}{mask_to_num(x.specification.mask)}"
    for x in simulator_inst.configurations[0].measurements
    if x.name in simulator_inst.measurement_names[0]
}
stat_names = {
    x.name: f"{x.metabolite_name}{mask_to_num(x.specification.mask)}"
    for x in simulator_stat.configurations[0].measurements
    if x.name in simulator_stat.measurement_names[0]
}
...
for sample_i in range(len(samples)):
    ...
    diffs.append(m)
print(
    f"Maximum difference between any INST and the corresponding STAT measurement: {np.max(diffs):.2e}"
)

plt.figure(figsize=(6.4, 4.8))
plt.hist(
    diffs,
    bins=np.logspace(-13, -5, 25),
