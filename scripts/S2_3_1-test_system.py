...
def global_error(jac, rhs, solution):
    ...
    b = Function("b")
    system = [
        Derivative(b(x), x) - jac[0, 0] * b(x) - jac[0, 1] * a(x),
        Derivative(a(x), x) - jac[1, 0] * b(x) - jac[1, 1] * a(x) - rhs,
    ]
    result = dsolve(system, ics={a(0.0): 0.01109, b(0.0): 0.01109})
    errors = np.linalg.norm(
        [
            np.array(
                [
                    float(result[0].rhs.evalf(subs={x: t})),
...
system = [
    Derivative(b(x), x) + (tau + 1) * b(x) - (tau + 1) * a(x),
...
for name in ["bdf", "sdirk"]:
    ...
    for j in [3, 6, 9]:
        simulator.builder.solver.relative_tolerance = np.power(10.0, -j)
        simulator.builder.solver.absolute_tolerance = np.power(10.0, -(j + 3))
        cond, errors = [], []
        print(f"\tsolving for solver tolerance 1e{-j}")
        for i in np.linspace(1, 3, 25):
            val = 10**i
            new_params = [1.0, val, 1.0, 1 / val]
            casc_sys = simulator.builder.build(
                simulator.parameter_space.compute_parameters(new_params)
            )
            ...
            errors.append(
                global_error(jac, level_sys.evaluate_inhomogeneity(0.0)[1], solution[0])
            )
...
for name in data:
    for j in data[name]:
        ...
        plt.loglog(
            cond,
            errors,
            label=f"{display_name}: " + "$tol_{rel} = 10^{-" + str(j) + "}$",
