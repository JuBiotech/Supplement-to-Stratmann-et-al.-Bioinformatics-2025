...
def time_measurement_simulation(
    measurements, network_data, exp_config, samples, method="emu"
):
    ...
    new_simulator = x3cflux.create_simulator_from_data(
        network_data, new_exp_config, sim_method=method
    )
    num_unknowns = 0
    cascaded_system = new_simulator.builder.build(
        new_simulator.parameter_space.compute_parameters(samples[0])
    )
    ...
    for k in range(samples.shape[1]):
        ...
        loc_times[k] = (time.perf_counter() - start) * 1000.0
...
def time_increasing_measurement_simulations(network_data, config, samples):
    ...
    for type in ["Tandem MS", "MS"]:
        ...
        cumo_times[0] = time_measurement_simulation(
            meas_list[:n_param_meas], network_data, config, samples, "cumomer"
        )
        emu_times[0] = time_measurement_simulation(
            meas_list[:n_param_meas], network_data, config, samples, "emu"
        )
        ...
        for i in range(n_param_meas, len(config.measurements)):
            ...
            print(
                f"\t\tAdding measurement {i + 1 - n_param_meas}/{len(config.measurements) - n_param_meas}: {curr_meas.name}"
            )

            if type == "Tandem MS":
                n = curr_meas.num_atoms
                m = n - 1
                spec = x3cflux.MSMSSpecification(
                    "1" * n,
                    "0" + "1" * m,
                ...
                data = x3cflux.MeasurementDataSet(
                    ts, len(ts) * [np.zeros(n * m)], len(ts) * [np.zeros(n * m)], []
                )
                incr_meas_list.append(
                    x3cflux.MSMSMeasurement(
                        curr_meas.name,
                        curr_meas.auto_scalable,
                        curr_meas.metabolite_name,
                        curr_meas.num_atoms,
                        spec,
                        data,
                    )
            ...
            cumo_times[i - n_param_meas + 1] = time_measurement_simulation(
                incr_meas_list, network_data, config, samples, "cumomer"
            )
            emu_times[i - n_param_meas + 1] = time_measurement_simulation(
                incr_meas_list, network_data, config, samples, "emu"
            )
...
def plot_solution_times(msms_times_stat, ms_times_stat, msms_times_inst, ms_times_inst):
    ...
    fig, axs = plt.subplots(
        2, 2, figsize=(figsize[0] * 2, figsize[1] * 2), sharex="col", sharey="row"
    )
    ...
    x = np.linspace(
        ms_times_stat[0][1:, 0].min() - 200, ms_times_stat[0][1:, 0].max() + 100, 100
    )
    axs[0, 0].plot(x, result.slope * x + result.intercept, ls="--", color="tab:blue")
    axs[0, 0].errorbar(
        ms_times_stat[1][1:, 0],
        ms_times_stat[1][1:, 1],
        yerr=ms_times_stat[1][1:, 2],
        label="EMU",
        marker="o",
        ls="",
        color="tab:orange",
    ...
    x = np.linspace(
        ms_times_stat[1][1:, 0].min() - 50, ms_times_stat[1][1:, 0].max() + 100, 100
    )
    ...
    axs[0, 1].errorbar(
        msms_times_stat[0][1:, 0],
        msms_times_stat[0][1:, 1],
        yerr=msms_times_stat[0][1:, 2],
        label="cumomer",
        marker="o",
        ls="",
        color="tab:blue",
    ...
    print(f"\tcumomer: R² = {result.rvalue:.4f}")
    x = np.linspace(
        msms_times_stat[0][1:, 0].min() - 100,
        msms_times_stat[0][1:, 0].max() + 100,
        100,
    )
    axs[0, 1].plot(x, result.slope * x + result.intercept, ls="--", color="tab:blue")
    axs[0, 1].errorbar(
        msms_times_stat[1][1:, 0],
        msms_times_stat[1][1:, 1],
        yerr=msms_times_stat[1][1:, 2],
        label="EMU",
        marker="o",
        ls="",
        color="tab:orange",
    ...
    print(f"\tEMU    : R² = {result.rvalue:.4f}")
    x = np.linspace(
        msms_times_stat[1][1:, 0].min() - 100,
        msms_times_stat[1][1:, 0].max() + 100,
        100,
    )
    ...
    print(f"\tcumomer: R² = {result.rvalue:.4f}")
    x = np.linspace(
        ms_times_inst[0][1:, 0].min() - 200, ms_times_inst[0][1:, 0].max() + 100, 100
    )
    axs[1, 0].plot(x, result.slope * x + result.intercept, ls="--", color="tab:blue")
    axs[1, 0].errorbar(
        ms_times_inst[1][1:, 0],
        ms_times_inst[1][1:, 1],
        yerr=ms_times_inst[1][1:, 2],
        label="EMU",
        marker="o",
        ls="",
        color="tab:orange",
    ...
    print(f"\tEMU    : R² = {result.rvalue:.4f}")
    x = np.linspace(
        ms_times_inst[1][1:, 0].min() - 50, ms_times_inst[1][1:, 0].max() + 100, 100
    )
    ...
    print("INST MSMS")
    axs[1, 1].errorbar(
        msms_times_inst[0][1:, 0],
        msms_times_inst[0][1:, 1],
        yerr=msms_times_inst[0][1:, 2],
        label="cumomer",
        marker="o",
        ls="",
        color="tab:blue",
    ...
    x = np.linspace(
        msms_times_inst[0][1:, 0].min() - 100,
        msms_times_inst[0][1:, 0].max() + 100,
        100,
    )
    axs[1, 1].plot(x, result.slope * x + result.intercept, ls="--", color="tab:blue")
    axs[1, 1].errorbar(
        msms_times_inst[1][1:, 0],
        msms_times_inst[1][1:, 1],
        yerr=msms_times_inst[1][1:, 2],
        label="EMU",
        marker="o",
        ls="",
        color="tab:orange",
    ...
    x = np.linspace(
        msms_times_inst[1][1:, 0].min() - 100,
        msms_times_inst[1][1:, 0].max() + 100,
        100,
    )
...
def plot_total_essential_dimension(
    msms_times_stat, ms_times_stat, msms_times_inst, ms_times_inst
):
    figsize = plt.rcParams["figure.figsize"]
    fig, axs = plt.subplots(
        2, 2, figsize=(figsize[0] * 2, figsize[1] * 2), sharex="col", sharey="row"
    ...
    for i in range(1, ms_times_stat[0].shape[0]):
        axs[0, 0].bar(
            3 * i, ms_times_stat[0][i, 1], width=1, color="tab:blue", label="cumomer"
        )
        axs[0, 0].bar(
            3 * i + 1, ms_times_stat[1][i, 1], width=1, color="tab:orange", label="EMU"
        )
        if i == 1:
            axs[0, 0].legend()
    axs[0, 0].errorbar(
        [3 * i for i in range(1, ms_times_stat[0].shape[0])],
        ms_times_stat[0][1:, 1],
        yerr=ms_times_stat[0][1:, 2],
        c="black",
        ls="",
    ...
    axs[0, 0].errorbar(
        [3 * i + 1 for i in range(1, ms_times_stat[1].shape[0])],
        ms_times_stat[1][1:, 1],
        yerr=ms_times_stat[1][1:, 2],
        c="black",
        ls="",
    ...
    for i in range(1, msms_times_stat[0].shape[0]):
        axs[0, 1].bar(
            3 * i, msms_times_stat[0][i, 1], width=1, color="tab:blue", label="cumomer"
        )
        axs[0, 1].bar(
            3 * i + 1,
            msms_times_stat[1][i, 1],
            width=1,
            color="tab:orange",
            label="EMU",
        )
    axs[0, 1].errorbar(
        [3 * i for i in range(1, msms_times_stat[0].shape[0])],
        msms_times_stat[0][1:, 1],
        yerr=msms_times_stat[0][1:, 2],
        c="black",
        ls="",
    ...
    axs[0, 1].errorbar(
        [3 * i + 1 for i in range(1, msms_times_stat[1].shape[0])],
        msms_times_stat[1][1:, 1],
        yerr=msms_times_stat[1][1:, 2],
        c="black",
        ls="",
    ...
    for i in range(1, ms_times_inst[0].shape[0]):
        axs[1, 0].bar(
            3 * i, ms_times_inst[0][i, 1], width=1, color="tab:blue", label="cumomer"
        )
        axs[1, 0].bar(
            3 * i + 1, ms_times_inst[1][i, 1], width=1, color="tab:orange", label="EMU"
        )
    axs[1, 0].errorbar(
        [3 * i for i in range(1, ms_times_inst[0].shape[0])],
        ms_times_inst[0][1:, 1],
        yerr=ms_times_inst[0][1:, 2],
        c="black",
        ls="",
    ...
    axs[1, 0].errorbar(
        [3 * i + 1 for i in range(1, ms_times_inst[1].shape[0])],
        ms_times_inst[1][1:, 1],
        yerr=ms_times_inst[1][1:, 2],
        c="black",
        ls="",
    ...
    for i in range(1, msms_times_inst[0].shape[0]):
        axs[1, 1].bar(
            3 * i, msms_times_inst[0][i, 1], width=1, color="tab:blue", label="cumomer"
        )
        axs[1, 1].bar(
            3 * i + 1,
            msms_times_inst[1][i, 1],
            width=1,
            color="tab:orange",
            label="EMU",
        )
    axs[1, 1].errorbar(
        [3 * i for i in range(1, msms_times_inst[0].shape[0])],
        msms_times_inst[0][1:, 1],
        yerr=msms_times_inst[0][1:, 2],
        c="black",
        ls="",
    ...
    axs[1, 1].errorbar(
        [3 * i + 1 for i in range(1, msms_times_inst[1].shape[0])],
        msms_times_inst[1][1:, 1],
        yerr=msms_times_inst[1][1:, 2],
        c="black",
        ls="",
...
simulator = x3cflux.create_simulator_from_data(
    data.network_data, data.configurations[config_index_stat]
)
...
problem = hopsy.add_box_constraints(
    problem, 12 * [-200.0] + 23 * [1e-6], 12 * [200.0] + 23 * [1000]
)
print(f"\tDrawing {n_samples} samples with hopsy")
samples = hopsy.sample(
    hopsy.MarkovChain(problem, hopsy.UniformCoordinateHitAndRunProposal),
    hopsy.RandomNumberGenerator(42),
    n_samples=n_samples,
    thinning=int(60**2 // 6),
)[1].reshape((n_samples, -1))

msms_times_stat, ms_times_stat = time_increasing_measurement_simulations(
    data.network_data, data.configurations[config_index_stat], samples
)

print("Evaluating INST Problem")
simulator = x3cflux.create_simulator_from_data(
    data.network_data, data.configurations[config_index_inst]
)
ineq_sys = simulator.parameter_space.inequality_system
problem = hopsy.Problem(ineq_sys.matrix, ineq_sys.bound)
problem = hopsy.add_box_constraints(
    problem,
    12 * [-200.0] + 23 * [1e-6] + 56 * [1e-6],
    12 * [200.0] + 23 * [1000] + 56 * [1e-1],
)
...
msms_times_inst, ms_times_inst = time_increasing_measurement_simulations(
    data.network_data, data.configurations[config_index_inst], samples
)

plot_total_essential_dimension(
    msms_times_stat, ms_times_stat, msms_times_inst, ms_times_inst
)
