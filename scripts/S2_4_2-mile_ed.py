...
def benchmark_ed(network_data, configuration, batch_sizes, substr_mix_collections, parallel_batch_size):
    ...
    for i, batch_size in enumerate(batch_sizes):
        ...
        for substrate in configuration.substrates:
            if substrate.metabolite_name not in [mix.metabolite_name for mix in
                                                 substr_mix_collections[:1]]:
        ...
        for j in range(int(batch_size / (parallel_batch_size + 1)) + 1):
            simulator = x3cflux.create_simulator_from_inputs(network_data, configuration,
                                                             substrates_mixtures[
                                                             j * size:min(batch_size, (j + 1) * size)],
                                                             fixed_substrates)
            last_params = x3cflux.get_parameters(simulator.parameter_space, configuration.parameter_entries)
        ...
        for j in range(batch_size):
            ...
            simulator = x3cflux.create_simulator_from_data(network_data, mix_meas_config)
            last_params = x3cflux.get_parameters(simulator.parameter_space,
                                                 configuration.parameter_entries)
            start = time.perf_counter()
            simulator.compute_multi_jacobians(last_params)
...
def plot_ed_benchmark(parallel_times, sequential_times, batch_sizes, postfix):
    slope, intercept, _, _, _ = stats.linregress(np.log10(batch_sizes), np.log10(parallel_times))
    x = np.arange(0, 4.3, step=0.1)
    plt.plot(np.power(10, x), np.power(10, slope * x + intercept),
             linestyle="--", color="tab:blue")
    plt.errorbar(batch_sizes, parallel_times, label=f"batch-wise ({slope:0.1f} * x - {-intercept:0.1f})",
                 marker="o", ls="", color="tab:blue")
    slope, intercept, _, _, _ = stats.linregress(np.log10(batch_sizes), np.log10(sequential_times))
    x = np.arange(0, 4.3, step=0.1)
    plt.plot(np.power(10, x), np.power(10, slope * x + intercept),
             linestyle="--", color="tab:orange")
    plt.errorbar(batch_sizes, sequential_times, label=f"sequential ({slope:0.1f} * x - {-intercept:0.1f})",
