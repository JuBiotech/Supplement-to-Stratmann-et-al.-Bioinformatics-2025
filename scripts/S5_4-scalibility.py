...
simulator_base = x3cflux.create_simulator_from_data(
    data.network_data, data.configurations[config_index]
)
params = x3cflux.get_parameters(
    simulator_base.parameter_space, data.configurations[config_index].parameter_entries
)
...
print(
    f"a tolerance decrease of 1 order of magnitude leads to a {(np.power(10, model.coef_[0]) - 1) * 100:.0f}% runtime increase"
)
...
plt.errorbar(
    xs,
    times.mean(axis=0).values,
    yerr=times.std(axis=0).values,
    fmt="o",
    color=C_X3,
    label="13CFLUX(v3)",
)
